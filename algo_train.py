...
def sup_trn(net,
    ...
    if opt_dict['solver'] == 'SGDM':
        ...
        opt = #TODO implement Adam optimizer
    else:
    ...
    criterion = #TODO implement cross-entropy loss function
    ...
    for epoch in range(opt_dict['num_epochs']): # Iterate over epochs
        ...
        for idx, (X,label) in enumerate(trn_loader):
            ...
            opt. #TODO zero all gradient in the optimizer
            pred = #TODO perform a forward pass
            train_loss = #TODO compute mini-batch loss
            train_loss. #TODO perform a backward pass
            opt. #TODO update parameetrs

            pred_cls = pred.argmax(dim=-1)
            train_acc = #TODO compute prediction accuracy for a single mini-batch
            ...
            if np.mod(itr, opt_dict['val_cycle']) == 0 or itr == itr_last:
                ...
                with torch.no_grad(): # MAke sure no gradients are computed
                    ...
                    val_loss = 0
                    val_acc = 0
                    for #TODO iterate over val_loader
                        X = #TODO prepare input image mini-batch
                        label = #TODO prepare label mini-batch

                        pred = #TODO forward pass
                        val_loss += #TODO calculate and accumelate loss (read about .item() in PyTorch)
                    ...
                    print('Epoch {0:}--Validation loss {1:6.5f} and accuracy {2:6.5f}'.#TODO print the information
                    )
